...
public class PersonelPaneli extends JFrame {
    ...
    private void adisyonEkraniAc(String siparisTuru, String baslikIsmi) {
            ...
            if (cevap != null && cevap.startsWith("KASA_VERI|") && cevap.length() > 10) {
                ...
                for (String s : siparisler) {
                    ...
                    if (d.length >= 5 && d[1].equals(baslikIsmi)) {
                        ...
                        // ONARILMIŞ GÜVENLİ FİYAT BULMA MEKANİZMASI
        ...
        JTextField txtMusteriTel = new JTextField(12); JTextField txtMusteriAdres = new JTextField(25);
        ...
        JPanel pnlAlt = new JPanel(new BorderLayout()); lblToplamTutar = new JLabel("Toplam: 0.0 TL"); lblToplamTutar.setFont(new Font("Arial", Font.BOLD, 14)); 
        recalculateTotal();
        ...
        btnMutfagaGonder.addActionListener(e -> {
            ...
            if(siparisTuru.equals("EVE_SERVIS")) fis.append("<b style='font-size:13px; color:red;'>[").append(zaman).append("] 🛵 EVE SERVİS</b><br>").append(alanKisiHTML).append("Müşteri: <b>").append(musteriAdi).append("</b> (").append(txtMusteriTel.getText()).append(")<br>Adres: ").append(txtMusteriAdres.getText()).append("<br><hr>");
            else if (siparisTuru.equals("PAKET")) fis.append("<b style='font-size:13px; color:blue;'>[").append(zaman).append("] 📦 GEL-AL PAKET</b><br>").append(alanKisiHTML).append("Müşteri: <b>").append(musteriAdi).append("</b><br><hr>");
            else { 
                ...
                siparisZamanlari.putIfAbsent(baslikIsmi, System.currentTimeMillis()); masaDurumlari.put(baslikIsmi, "HAZIRLANIYOR"); 
            }
            ...
            JOptionPane.showMessageDialog(adisyonDialog, "Sipariş mutfağa başarıyla iletildi!"); adisyonDialog.dispose();
        });
        ...
        split.setLeftComponent(pnlUrunler); split.setRightComponent(pnlSepet); adisyonDialog.add(split, BorderLayout.CENTER); adisyonDialog.setVisible(true);
    }
    ...
    private void recalculateTotal() {
        double total = gecerliOncekiTutar;
        for (int i = 0; i < sepetTableModel.getRowCount(); i++) {
            String fiyatStr = sepetTableModel.getValueAt(i, 2).toString();
            total += guvenliDoubleCevir(fiyatStr);
        }
        if (lblToplamTutar != null) {
            lblToplamTutar.setText("Toplam: " + total + " TL");
        }
    }
    ...
    private void urunOzellestirmePenceresi(String urunAd, double tabanFiyat, String malzemelerStr) {
        ...
        btnSepeteEkle.addActionListener(e -> {
            ...
            sepetTableModel.addRow(new Object[]{urunAd, 1, sonFiyat, sonNot}); recalculateTotal(); d.dispose();
        });
