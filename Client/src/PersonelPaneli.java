...
public class PersonelPaneli extends JFrame {
    ...
    private double guvenliDoubleCevir(String veri) {
        try {
            if (veri == null || veri.trim().isEmpty()) return 0.0;
                    ...
                    for (int i = 1; i < parcalar.length; i++) {
                        ...
                        masaButon.addActionListener(e -> {
                            ...
                            if (durum.equals("BOS") && bugunkuRezervasyonlar.containsKey(masaIsmi)) {
                                ...
                                if(onay == JOptionPane.YES_OPTION) adisyonEkraniAc("MASA", masaIsmi);
                                return;
                            }
                ...
                if(cevap.length() > 14) {
                    ...
                    for (String m : masalar) {
                        ...
                        if (detay.length == 3) {
                            ...
                            if(durum.equals("BEKLEMEDE") || durum.equals("HAZIRLANIYOR")) yeniDurumlar.put(masaAdi, "HAZIRLANIYOR");
                            else if (durum.equals("HAZIR") || durum.equals("TESLIM_EDILDI")) yeniDurumlar.put(masaAdi, "TESLIM_EDILDI");
                ...
                if (cevap != null && cevap.startsWith("KASA_VERI|") && cevap.length() > 10) {
                    ...
                    for (String s : siparisler) {
                        ...
                        if (d.length >= 5 && d[1].equals(baslikIsmi)) {
                            oncekiVeriler[0] = d[2]; // musteriAdi
                            String html = d[4];
                            htmlBuilder.append("<div style='border-bottom: 1px dashed #ccc; padding-bottom: 5px; margin-bottom: 5px;'>").append(html).append("</div>");

                            int fBas = html.indexOf("<b>", 0);
                            if (fBas != -1) {
                                int fEnd = html.indexOf("</b>", fBas);
                                if (fEnd != -1) {
                                    String fiyatStr = html.substring(fBas + 3, fEnd);
                                    tutar += guvenliDoubleCevir(fiyatStr);
                                }
                            }
                        }
                    }
                    oncekiVeriler[1] = htmlBuilder.toString();
                    oncekiVeriler[2] = String.valueOf(tutar);
                }
            }

            // Arayüzü EDT üzerinde oluştur
            SwingUtilities.invokeLater(() -> {
                final JDialog adisyonDialog = new JDialog(this, "Sipariş: " + baslikIsmi, true);
                adisyonDialog.setSize(1200, 750);
                adisyonDialog.setLayout(new BorderLayout());
                adisyonDialog.setLocationRelativeTo(this);

                gecerliOncekiTutar = Double.parseDouble(oncekiVeriler[2]);
                ...
                if (!oncekiVeriler[0].isEmpty()) txtMusteriAd.setText(oncekiVeriler[0]);
                ...
                JLabel lblEkAciklama = new JLabel("Ek Açıklama:"); lblEkAciklama.setForeground(Color.WHITE); lblEkAciklama.setFont(new Font("Arial", Font.BOLD, 14));
                ...
                JPanel pnlKategoriler = new JPanel(new GridLayout(0, 1, 5, 5)); pnlKategoriler.setBackground(new Color(52, 73, 94)); pnlKategoriler.setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 10));
                JScrollPane scrollKategori = new JScrollPane(pnlKategoriler); scrollKategori.setPreferredSize(new Dimension(160, 0)); scrollKategori.setBorder(BorderFactory.createMatteBorder(0, 0, 0, 1, Color.GRAY));
                JPanel pnlUrunListesi = new JPanel(new GridLayout(0, 3, 10, 10)); pnlUrunListesi.setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 10));
                    ...
                    if (cevap != null && cevap.startsWith("KAT_LISTESI")) {
                        ...
                        SwingUtilities.invokeLater(() -> {
                            for (int i = 1; i < parcalar.length; i++) {
                                ...
                                b.addActionListener(e -> yukleUrunleriButonOlarak(katAdi, pnlUrunListesi)); pnlKategoriler.add(b);
                            }
                ...
                if (ekSiparisMi && !oncekiVeriler[1].isEmpty()) {
                    JEditorPane txtEski = new JEditorPane(); txtEski.setContentType("text/html");
                    txtEski.setText("<html><div style='font-family: Arial; font-size: 11px;'>" + oncekiVeriler[1] + "</div></html>");
                    txtEski.setEditable(false); txtEski.setBackground(new Color(250, 250, 250));
                    JScrollPane scrollEski = new JScrollPane(txtEski); scrollEski.setPreferredSize(new Dimension(0, 200));
                ...
                JTable sepetTablosu = new JTable(sepetTableModel); sepetTablosu.getColumnModel().getColumn(3).setPreferredWidth(200); pnlSepet.add(new JScrollPane(sepetTablosu), BorderLayout.CENTER);
                ...
                hesaplaToplam(); // Önceki tutarı yansıtmak için
                ...
                btnMutfagaGonder.addActionListener(e -> {
                    ...
                    if(musteriAdi.isEmpty()) { JOptionPane.showMessageDialog(adisyonDialog, "Lütfen Müşteri ismini giriniz!", "Hata", JOptionPane.ERROR_MESSAGE); return; }
                    ...
                    StringBuilder fis = new StringBuilder("<html>"); String zaman = new SimpleDateFormat("HH:mm").format(new Date()); String alanKisiHTML = "Siparişi Alan: <b>" + aktifPersonel + "</b><br>";

                    if(siparisTuru.equals("EVE_SERVIS")) {
                        fis.append("<b style='font-size:13px; color:red;'>[").append(zaman).append("] 🛵 EVE SERVİS</b><br>").append(alanKisiHTML).append("Müşteri: <b>").append(musteriAdi).append("</b> (").append(txtMusteriTel.getText()).append(")<br>").append("Adres: ").append(txtMusteriAdres.getText()).append("<br>");
                        if(!txtEkAciklama.getText().trim().isEmpty()) { fis.append("Ek Açıklama: <i>").append(txtEkAciklama.getText().trim()).append("</i><br>"); }
                        fis.append("<hr>");
                    } else if (siparisTuru.equals("PAKET")) {
                        fis.append("<b style='font-size:13px; color:blue;'>[").append(zaman).append("] 📦 GEL-AL PAKET</b><br>").append(alanKisiHTML).append("Müşteri: <b>").append(musteriAdi).append("</b><br><hr>");
                    } else {
                        String tag = ekSiparisMi ? "➕ EK SİPARİŞ" : "🍽 YENİ MASA";
                        fis.append("<b style='font-size:13px; color:green;'>[").append(zaman).append("] ").append(tag).append(" (").append(baslikIsmi).append(")</b><br>").append(alanKisiHTML).append("Müşteri: <b>").append(musteriAdi).append("</b><br><hr>");
                    }

                    for (int i = 0; i < sepetTableModel.getRowCount(); i++) {
                        String urun = sepetTableModel.getValueAt(i, 0).toString(); String adet = sepetTableModel.getValueAt(i, 1).toString(); String notlar = sepetTableModel.getValueAt(i, 3).toString();
                    ...
                    final String finalFis = fis.toString();
                    btnMutfagaGonder.setEnabled(false);
                    btnMutfagaGonder.setText("Gönderiliyor...");

                    new Thread(() -> {
                        sunucuyaKomutGonderVeCevapAl("SIPARIS_OLUSTUR|" + baslikIsmi + "|" + musteriAdi + "|" + finalFis);
                        SwingUtilities.invokeLater(() -> {
                            if (siparisTuru.equals("MASA")) {
                                siparisZamanlari.putIfAbsent(baslikIsmi, System.currentTimeMillis());
                                masaDurumlari.put(baslikIsmi, "HAZIRLANIYOR");
                            }
                            if (bugunkuRezervasyonlar.containsKey(baslikIsmi)) bugunkuRezervasyonlar.remove(baslikIsmi);
                ...
                pnlAlt.add(lblToplamTutar, BorderLayout.WEST); pnlAlt.add(btnMutfagaGonder, BorderLayout.EAST); pnlSepet.add(pnlAlt, BorderLayout.SOUTH);
            ...
            if(cevap != null && cevap.startsWith("URUN_LISTESI_DETAYLI")) {
                ...
                SwingUtilities.invokeLater(() -> {
                    ...
                    for(int i = 1; i < urunler.length; i++) {
                        String[] d = urunler[i].split(";"); String ad = d[0]; 
                        ...
                        btnUrun.addActionListener(e -> urunOzellestirmePenceresi(ad, fiyat, malzemeler)); pnlListe.add(btnUrun);
                    }
    ...
    private void urunOzellestirmePenceresi(String urunAd, double tabanFiyat, String malzemelerStr) {
        JDialog d = new JDialog(this, urunAd + " - Özelleştir", true); d.setSize(450, 550); d.setLayout(new BorderLayout()); d.setLocationRelativeTo(this);
        ...
        if(!malzemelerStr.isEmpty() && !malzemelerStr.equals("null")) {
            ...
            for(String m : malzemeler) {
                ...
                if(detay.length == 3) {
                    ...
                    if (standartMi) { JCheckBox cb = new JCheckBox(detay[0], true); cb.setName(m); pnlCikarilabilir.add(cb); } 
                    else { JCheckBox cb = new JCheckBox(detay[0] + " (+ " + ekUcret + " TL)", false); cb.setName(m); pnlEkstralar.add(cb); }
                }
            }
        }
        ...
        btnSepeteEkle.addActionListener(e -> {
            double sonFiyat = tabanFiyat; ArrayList<String> notListesi = new ArrayList<>(); 
            for (Component comp : pnlCikarilabilir.getComponents()) { if (comp instanceof JCheckBox) { JCheckBox cb = (JCheckBox) comp; if(!cb.isSelected()) notListesi.add("- " + cb.getName().split(":")[0]); } }
            for (Component comp : pnlEkstralar.getComponents()) { if (comp instanceof JCheckBox) { JCheckBox cb = (JCheckBox) comp; if(cb.isSelected()) { String[] mDetay = cb.getName().split(":"); notListesi.add("+ " + mDetay[0]); sonFiyat += guvenliDoubleCevir(mDetay[2]); } } }
            if(!txtNot.getText().trim().isEmpty()) notListesi.add("Not: " + txtNot.getText().trim());
            String sonNot = String.join(" | ", notListesi); if(sonNot.isEmpty()) sonNot = "Standart";
            sepetTableModel.addRow(new Object[]{urunAd, 1, sonFiyat, sonNot}); hesaplaToplam(); d.dispose();
        });
    ...
    public void masayiSifirla(String masaAdi) {
        siparisZamanlari.remove(masaAdi); masaDurumlari.put(masaAdi, "BOS");
        if(masalarPaneli != null) {
            for (Component c : masalarPaneli.getComponents()) {
                if (c instanceof JButton && c.getName() != null && c.getName().equals(masaAdi)) { ((JButton) c).setText(masaAdi); c.setBackground(new Color(236, 240, 241)); }
            }
        }
    }
    ...
    private void masaRenkGuncelleyiciyiBaslat() {
        Timer masaTimer = new Timer(5000, e -> { 
            ...
            if(masalarPaneli != null) {
                for (Component c : masalarPaneli.getComponents()) {
                    if (c instanceof JButton) guncelleMasaButonu((JButton) c, suan);
                }
            }
        });
        masaTimer.start();
    ...
    private void guncelleMasaButonu(JButton btn, long suan) {
        ...
        if (durum.equals("BOS")) {
            ...
            if (rezBilgisi != null) {
                ...
                btn.setText(mName); btn.setBackground(new Color(236, 240, 241)); 
            }
        } else {
